...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[], disable_radar=False):  # pylint: disable=dangerous-default-value
    ...
    if candidate == CAR.PRIUS:
      ...
      set_lat_tune(ret.lateralTuning, LatTunes.TORQUE, MAX_LAT_ACCEL=1.31, FRICTION=0.07)
    ...
    # if Params().get_bool("UseStockLong"):
  ...
  def _update(self, c):
    ...
    if self.last_cruise_actual_enabled and not ret.cruiseActualEnabled:
      events.add(EventName.communityFeatureDisallowedDEPRECATED)
    elif ret.cruiseState.enabled != ret.cruiseActualEnabled:
      events.add(EventName.gasPressedOverride)
    self.last_cruise_actual_enabled = ret.cruiseActualEnabled
