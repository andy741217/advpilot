...
MAX_FRAMES_ABOVE_RATE_THRESHOLD = 18
...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    predicted_steering_fault = self.frames_above_rate_threshold > MAX_FRAMES_ABOVE_RATE_THRESHOLD

    # Cut steering while we're in a known fault state (2s) or about to be
    if not CC.latActive or predicted_steering_fault or (abs(CS.out.steeringAngleDeg) > 150 and CS.CP.carFingerprint in [CAR.RAV4H, CAR.PRIUS]):
      apply_steer = 0
      apply_steer_req = 0
    else:
      apply_steer_req = 1
