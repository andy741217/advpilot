...
def thermald_thread(end_event, hw_queue):
  ...
  # use_old_panda = params.get_bool("UseOldPanda")

  # use temp to check if the device has battery or not, report < 0 when no battery.
  has_battery = False
  try:
    with open("/sys/class/power_supply/battery/temp") as f:
      has_battery = int(float(f.read())) < 0
  except Exception:
    pass
  ...
  while not end_event.is_set():
    ...
    # check usb_present to fix not going offroad on "EON/LEON + battery - Comma Power"
    #if use_old_panda or has_battery:
    if has_battery:
    ...
    if started_seen and off_ts is not None and (sec_since_boot() - off_ts) > 10:
